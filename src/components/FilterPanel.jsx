  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <button onClick={() => set('showR18', !filters.showR18)}
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg transition-all text-left"
            style={{ background: filters.showR18 ? 'rgba(154,64,64,0.12)' : 'var(--bg-overlay)', border: '1px solid ' + (filters.showR18 ? 'rgba(154,64,64,0.4)' : 'var(--border)'), color: filters.showR18 ? '#ffaaaa' : 'var(--text-muted)', fontSize: '11px', fontFamily: 'DM Sans, sans-serif' }}>
            <span style={{ fontSize: '10px', fontWeight: '700', fontFamily: 'DM Sans, sans-serif', flexShrink: 0 }}>R18</span>
            {filters.showR18 ? 'R18 Visible' : 'Show R18 Content'}
          </button>
      ...
      <div>
        ...
        {filters.tags?.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {filters.tags.map(t => (
              <button key={t} onClick={() => toggleArr('tags', t)}
                style={{ ...chip(true), fontSize: '10px' }}>
                {t} ×
              </button>
            ))}
          </div>
      ...
      <div>
        ...
        <select style={inputStyle} value={filters.sortBy || 'updated_at'} onChange={e => set('sortBy', e.target.value)}>
          ...
          <option value="rss_last_item_date">Recently Updated (RSS)</option>
